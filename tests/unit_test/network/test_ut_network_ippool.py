...
def test_ip_pool_create_success(mocker):
    mock_login = mocker.patch('ucsmsdk.ucshandle.UcsHandle.login', autospec=True)
    mock_login.return_value = True
    mock_query_dn = mocker.patch('ucsmsdk.ucshandle.UcsHandle.query_dn', autospec=True)
    mock_query_dn.return_value = "org-root"
    mock_commit = mocker.patch('ucsmsdk.ucshandle.UcsHandle.commit', autospec=True)
    mock_commit.return_value = None
    mock_ip_pool = mocker.patch('ucsm_apis.network.ippool.IppoolPool')
    mock_add_mo = mocker.patch('ucsmsdk.ucshandle.UcsHandle.add_mo', autospec=True)
    mock_add_mo.return_value = None

    ippool.ip_pool_create(handle, "dummypool")

    mock_query_dn.assert_called_with(handle, "org-root")
    mock_ip_pool.assert_called_with(descr=None, name='dummypool', parent_mo_or_dn='org-root')
...
def test_ip_pool_exists_success(mocker):
    ...
    mock_login.return_value = True
    mock_query_dn = mocker.patch.object(UcsHandle, 'query_dn', autospec=True)
    mock_check_prop = mocker.patch.object(ippool.IppoolPool, 'check_prop_match', autospec=True)
    ...
    mock_get.assert_called_with(handle=handle, caller='ip_pool_exists', name= "dummypool", org_dn="org-root")
...
def test_ip_pool_modify_success(mocker):
    ...
    mock_get.assert_called_with(handle=handle, name="dummypool", org_dn="org-root", caller="ip_pool_modify")
    managed_object_mock.set_prop_multiple.assert_called_with(descr="This is a new pool")
...
def test_ip_pool_delete_success(mocker):
    ...
    mock_get.assert_called_with(handle=handle, name="dummypool", org_dn="org-root", caller="ip_pool_delete")
...
def test_ip_block_create_success(mocker):
    ...
    mock_query_dn = mocker.patch.object(UcsHandle, 'query_dn', autospec=True)
    mock_query_dn.return_value = "org-root/ip-pool-dummypool"
    mock_ip_pool_block = mocker.patch.object(ippool, 'IppoolBlock', autospec=True)
    mock_add_mo = mocker.patch.object(UcsHandle, 'add_mo', autospec=True)
    mock_commit = mocker.patch.object(UcsHandle, 'commit', autospec=True)

    ippool.ip_block_create(handle, "dummypool", start_ip="192.168.10.50", end_ip="192.168.10.60",
                           sm="255.255.255.0", gw="192.168.10.1")

    mock_query_dn.assert_called()
    mock_ip_pool_block.assert_called_with(parent_mo_or_dn="org-root/ip-pool-dummypool",
                                          def_gw="192.168.10.1", r_from="192.168.10.50",
